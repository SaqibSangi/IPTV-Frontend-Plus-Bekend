...
const Home = () => {
  ...
  const fetchGenres = async () => {
    setLoading(true);
    try {
      const response = await fetch("https://localhost/api/genres");
  ...
  const fetchGenreById = async (id) => {
    setLoading(true);
    try {
      const response = await fetch(`https://localhost/api/genres/${id}`);
      const result = await response.json();
  ...
  const handleDeleteGenre = async (id) => {
    ...
    try {
      const response = await fetch(`https://localhost/api/genres/${id}`, requestOptions);
  ...
  const handleAddGenre = async () => {
    ...
    try {
      const response = await fetch("https://localhost/api/genres", requestOptions);
      const result = await response.json();
      console.log(result, '------')
      if (response.ok) {
        setAlertType('success');
  ...
  const handleUpdateGenre = async () => {
    ...
    try {
      const response = await fetch(`https://localhost/api/genres/${editingId}`, requestOptions);
      const result = await response.json();
  ...
  return (
    <div className="home-container">
      ...
      {loading && <LoadingIndicator/>} {/* Show loader */}
      ...
      <table className="data-table">
        <thead>
          <tr>
            <th style={{color: '#000'}}>Add Genre</th>
          </tr>
          <tr>
            <th>
              <input
